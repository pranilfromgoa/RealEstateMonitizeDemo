...
export function InvestorTaxDocuments() {
    ...
    <Layout>
      ...
      <div className="ds-page-narrow">
        <div className="bg-purple-50 border border-purple-200 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <p className="text-sm text-purple-700">Tax documents are auto-generated at year-end showing all rental income and capital gains from Brick trading.</p>
        </div>
          ...
          <CardContent className="space-y-3">
            {myDocs.map(doc => (
              <div key={doc.id} className="flex items-center gap-4 p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${doc.status === 'available' ? 'bg-green-100' : 'bg-yellow-100'}`}>
                  <FileText size={20} className={doc.status === 'available' ? 'text-green-600' : 'text-yellow-600'} />
                </div>
